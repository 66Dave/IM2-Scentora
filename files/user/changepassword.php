<?php
session_start();
$step = isset($_SESSION['cp_verified']) && $_SESSION['cp_verified'] ? 2 : 1;
$errors = [];
$success = "";
$email = "";
...
if (isset($_POST['back_to_verify'])) {
    unset($_SESSION['cp_verified']);
    unset($_SESSION['cp_email']);
    $step = 1;
}

if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['back_to_verify'])) {
    if (isset($_POST['verify'])) {
        ...
        if (empty($errors)) {
            ...
            if ($stmt->execute()) {
                $success = "Password successfully changed. <a href='signin.php'>Sign in</a>";
                session_unset();
                session_destroy();
            } else {
...
<head>
    <meta charset="UTF-8">
    <title>Change Password | Scentora</title>
...
<body>
        ...
        <div class="form-container">
            ...
            <?php elseif ($step === 2): ?>
                ...
                <form method="POST" autocomplete="off" style="width:100%;">
                    ...
                    <div class="btn-row">
                        <button type="submit" name="back_to_verify" class="btn" style="background:#eee; color:#917489; margin-right:1rem;" formnovalidate>&#8592; Back</button>
                        <button type="submit" name="reset" class="btn">Change Password</button>
                    </div>
                </form>
                <?php endif; ?>
            <?php endif; ?>
    ...
    <div class="signin-link" style="position:fixed; bottom:1rem; left:50%; transform:translateX(-50%);">
        <a href="changepassword.php" style="color: var(--accent); text-decoration: underline; font-weight: 600;">Change Password</a>
    </div>
</body>
</html>