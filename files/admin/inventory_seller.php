...
/* Low Stock Filter Button Active State */
#lowStockFilter.active {
  background-color: #8663b9 !important;
  color: #fff !important;
}

/* Ensure low stock button is visible */
#lowStockFilter {
...
<body>
  ...
  <div class="inventory-main-container">
    ...
    <div class="inventory-controls">
      ...
      <button class="filter-btn" id="lowStockFilter">Low Stock</button>
...
function renderTable() {
  ...
  let filtered = inventory.filter(item => {
    ...
    if (showLowStockOnly) match = match && item.stock <= 12 && item.stock > 0; // Low stock filter
    return match;
  });

  // Debug logging
  if (showLowStockOnly) {
    console.log("Filtering for low stock. Total inventory:", inventory.length);
    console.log("Low stock items found:", filtered.length);
    console.log("Low stock items:", filtered.map(item => `${item.name}: ${item.stock}`));
  }
...
// Low Stock Filter
document.getElementById("lowStockFilter").onclick = () => {
  showLowStockOnly = !showLowStockOnly;
  const btn = document.getElementById("lowStockFilter");
  console.log("Low stock filter clicked, showLowStockOnly:", showLowStockOnly); // Debug log
  
  if (showLowStockOnly) {
    btn.style.backgroundColor = "#8663b9";
    btn.style.color = "#fff";
...
document.getElementById("nextPageBtn").onclick = function() {
  const filtered = inventory.filter(item => {
    ...
    if (filterDateUpdated) match = match && item.updated === filterDateUpdated;
    if (filterBrand) match = match && (item.brand && item.brand.toLowerCase().includes(filterBrand));
    if (showLowStockOnly) match = match && item.stock <= 12 && item.stock > 0;
    return match;
  });
...
document.addEventListener("DOMContentLoaded", () => {
  // Check for URL parameters to auto-filter low stock first
  ...
  if (urlParams.get('filter') === 'lowstock') {
    ...
    console.log("URL parameter detected: activating low stock filter"); // Debug log
  }
